...
public class CategoryPageController implements Initializable {
    ...
    static ArrayList<Room> categoryRooms = new ArrayList<>();
    static ArrayList<AnchorPane> categoryRoomsCompos = new ArrayList<>();
    static int n = 0;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            ...
            PreparedStatement howmany = con.prepareStatement("SELECT RoomID FROM rooms");
            ResultSet getid = howmany.executeQuery();
            while (getid.next()) {
                ...
                categoryRooms.add(new Room(hotelauthor, "", Integer.parseInt(roomcap), facility, Double.parseDouble(price)));
                try {
                    categoryRoomsCompos.add(FXMLLoader.load(Objects.requireNonNull(this.getClass().getResource("CategoryRoomsComponent.fxml"))));
                    ImageView iv = (ImageView) categoryRoomsCompos.get(n).getChildren().get(0);
...
//            con.close();
        ...
        try {
            ...
            PreparedStatement hwstate = com.prepareStatement("SELECT RoomID FROM rooms");
            ...
            while (ids.next()) {
                ...
                String price = getprice.getString(1);
                if (i % 3 == 0) {
                    vBox1.getChildren().add(categoryRoomsCompos.get(i));
                    HBox hbx = (HBox) categoryRoomsCompos.get(i).getChildren().get(5);
                    ...
                    Button buybtn = (Button) categoryRoomsCompos.get(i).getChildren().get(4);
                    ...
                    vBox2.getChildren().add(categoryRoomsCompos.get(i));
                    HBox hbx = (HBox) categoryRoomsCompos.get(i).getChildren().get(5);
                    Button btn = (Button) hbx.getChildren().get(0);
                    btn.setOnAction(event1 -> {
                        Method.notification(Alert.AlertType.INFORMATION, "Room's Author", "Author", "Author Of Room : " + hotelauthor);
                    ...
                    vBox3.getChildren().add(categoryRoomsCompos.get(i));
                    HBox hbx = (HBox) categoryRoomsCompos.get(i).getChildren().get(5);
                    Button btn = (Button) hbx.getChildren().get(0);
                    btn.setOnAction(event1 -> {
                        Method.notification(Alert.AlertType.INFORMATION, "Room's Author", "Author", "Author Of Room : " + hotelauthor);
                                        ...
                                        if (Double.parseDouble(bal) >= overallprice) {
                                            ...
                                            try {
                                                ...
                                                if (situation.next()) {
                                                    ...
                                                    RightPanel.ActiveRoom[0] = "Hotel name: " + author;
                                                    RightPanel.ActiveRoom[1] = "Number of night: " + nights;
                                                    RightPanel.ActiveRoom[2] = "price for " + nights + " night : " + Double.parseDouble(pricee) * Integer.parseInt(nights);
                                                }
                                            } catch (SQLException e) {
                                                e.printStackTrace();
                                            }
